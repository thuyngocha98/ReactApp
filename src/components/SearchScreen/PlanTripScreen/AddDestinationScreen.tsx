...
class AddDestinationScreen extends Component<Props, State> {
    ...
    searchFilterFunction = text => {
        let textSearch = text.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase(); // remove accent
        this.setState({
            values: text,
        });
        if(text == ""){
    ...
    render() {
        const RenderItem = ({item, index}) => (
            <ListItemSelectDestination item={item} navigation={this.props.navigation} onPressAdd={() => onPressHandler(index)}  />
        )
