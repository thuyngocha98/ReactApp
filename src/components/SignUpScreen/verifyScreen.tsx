...
class verifyScreen extends Component<Props, States> {
  ...
  _animationsColor = [...new Array(codeLength)].map(() => new Animated.Value(0));
  _animationsScale = [...new Array(codeLength)].map(() => new Animated.Value(1));
  dialogbox: any;
        ...
        <View style={verifyScreenStyles.inputWrapper}>
          ...
          >
            <View style={verifyScreenStyles.nextButton}>
              <Text style={verifyScreenStyles.nextButtonText}>Xác Nhận</Text>
            </View>
          </TouchableOpacity>
        </View>
        <DialogBox
